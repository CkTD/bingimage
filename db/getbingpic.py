...
def createdb(config):
	con = lite.connect('imgs.db')
	cur = con.cursor()
	cur.execute('CREATE TABLE imgs(startdate INTEGER PRIMARY KEY,copyright TEXT, url TEXT)')
	con.commit()
	cur.close()
	con.close()
...
def getbingbgimg(config):
	json_response = download('https://www.bing.com/HPImageArchive.aspx?format=js&idx=0&n=1',config['retry-time'])
	...
	try:
		cur = con.cursor()
		cur.execute("INSERT INTO imgs VALUES(%d,'%s','%s')"%(int(startdate),copyright,url))
		con.commit()
	except Exception as e:
		print(time.ctime(),'Cant save %s to sqlite,[ %s ]'%(startdate,e.args))
	else:
...
def get_config():
	...
	try:
		...
		config['pid-file'] = config.get('pid-file', '/var/run/%s.pid'%os.path.splitext(sys.argv[0])[0])
		config['log-file'] = config.get('log-file', '/var/log/%s.log'%os.path.splitext(sys.argv[0])[0])
